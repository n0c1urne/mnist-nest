    ...
    with gzip.open(name+'/snapshots'+str(nest.Rank()), 'wb') as f:
        ...
        for t in range(stimulus_count):
            ...
            input_rates = mnist_tools.calc_rates(
                x_train[t:t+1], pixel_samples, standardize_per_digit=True, strength=stimulus_strength) * params.rate
            ...
            # set rate for teacher poulation
            for j in range(teacher_stim_index,  teacher_stim_index+200):
                network.set_rate([j+1], params.rate *
                                 (1.0 + teacher_strength/100.0))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stimulus-count",
        type=int,
        help="number of stimuli (firt n digits of mnist)",
        required=True
    )
