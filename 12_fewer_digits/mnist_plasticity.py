...
def simulation(
    ...
    with gzip.open(name+'/snapshots'+str(nest.Rank()), 'wb') as f:
        ...
        for t in range(stimulus_count):
            for digit in [1, 3]:
                print("Stimulus", t, "Digit", y_train[digit])

                # ----- DIGIT INPUT -----------

                input_rates = mnist_tools.calc_rates(
                    x_train[digit:digit+1], pixel_samples, standardize_per_digit=True, strength=stimulus_strength)
                input_rates = input_rates.squeeze()
                input_rates[input_rates < 0] = 0  # ensure that no rate is negative

                stim = np.ones(4000)
                stim[0:2000] = input_rates
                
                teacher_stim_slice = slice(2000+y_train[digit]*200, 2000+(y_train[digit]+1)*200)

                stim[teacher_stim_slice] = (1.0 + teacher_strength/100.0)
                stim *= params.rate
